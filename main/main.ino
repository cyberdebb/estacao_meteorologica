...
WindIndicatorSensor win;

void setup() {
  // Initialize the pins
  analogReference(DEFAULT); // Reference tension to 3.3V
  pinMode(ANE_HALL_PIN, INPUT_PULLUP); // Set pin as input with internal pull-up
  pinMode(PLU_REED_PIN, INPUT_PULLUP);
  ...
  if (!bmp.begin(0x76 , &Wire)) {
    ...
    Serial.println(bmp_lib.sensorID(),16);
  ...
  bmp_lib.setSampling(Adafruit_BMP280::MODE_NORMAL,     /* Operating Mode. */
  ...
  // Attach interrupt
  attachInterrupt(digitalPinToInterrupt(ANE_HALL_PIN), ane.countRevolution, RISING);
  attachInterrupt(PLU_REED_PIN, plu.isr_rain, FALLING);
}
